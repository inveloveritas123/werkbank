...
KONZ="claude -p --model $BUILD_MODEL --dangerously-skip-permissions \"Konzipiere mit BMAD (Skills bmad-prd, bmad-create-architecture, bmad-create-epics-and-stories) aus diesem Brief: $BRIEF . Schreibe ALLE Dateien direkt ins AKTUELLE Verzeichnis (KEIN Unterordner anlegen). Erzeuge: SPEC.md (6 Pflichtfelder Ziel/Scope/Datenarten/Akzeptanz/Nicht-Ziele/Handoff, >=2 testbare Akzeptanzkriterien, Handoff-Checkliste [x]); ARCHITECTURE.md; TASKS.md mit self-contained Wellen (Dateien/Verbote/Smoke/Akzeptanz inline); DSGVO-Artefakte unter artefakte/ ($REQ); evidence/audit.log (JSONL). EU-Routing, keine Klartext-PII/Secrets, Modelle gepinnt (kein latest). CHANGELOG.md anlegen.\""
BAUEN="claude -p --model $BUILD_MODEL --dangerously-skip-permissions \"Arbeite die oberste offene Welle in TASKS.md test-first (RED->GREEN->REFACTOR). SELBST-VERIFIKATION vor dem Fertigmelden: lies GATE-REPORT.md und BACKLOG.md und behebe die roten Gate-Befunde gezielt (genannte Dateien/Fehler); fuehre dann selbst aus: ruff check ., mypy ., python3 -m unittest discover und behebe ALLE Befunde, bis sauber. Halte EU-Routing/PII-Freiheit/Audit-Log/Modell-Pinning ein. Gib NUR DANN exakt <promise>GRUEN</promise> aus, wenn ruff+mypy+Tests lokal sauber durchlaufen; sonst arbeite weiter.\""
