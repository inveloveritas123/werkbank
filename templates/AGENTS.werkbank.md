...
> **Kanonische Quelle der Wahrheit (SSoT)** für alle Coding-Agenten in diesem Projekt (Claude, Codex …).
> Werkzeugspezifische Dateien (`CLAUDE.md`) verweisen per `@AGENTS.md` hierher — **nicht** duplizieren.
> **Geltung: interner Einsatz, KEINE echten personenbezogenen Kundendaten** (siehe WERKBANK-Freigabe).

## Schichten-Abgrenzung (wichtig: keine Doppel-Config)
- **Methode = BMAD.** BMAD besitzt seine eigene Config unter `_bmad/` (Rollen, PRD/Architektur/Story-Flow).
  Diese Datei **referenziert** BMAD, sie **dupliziert** BMADs Config nicht. Quelle der Wahrheit für die
  *Methode* bleibt `_bmad/`.
- **Autonomie = kiln-Muster** (kein installiertes Tool): `.werkbank/STATE.md`, Ralph-Loop, persistente
  Minds, Drift-Pausegate.
- **Governance = WERKBANK** (`gates/`): deterministische Block-Gates. Das ist der Inhalt *dieser* Datei.
...
- **C4** E2E-Smoke: bevorzugt **agent-browser** (live gegen die laufende App), sonst Playwright. Ohne E2E-Config → SKIP.
